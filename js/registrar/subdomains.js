...
const rekeySubdomain = async (fqn, nameOwnerKey, subdomainOptions, network) => {
    ...
    const owner = subdomainOptions.newOwnerAddress ||
        transactions_2.publicKeyToAddress(constants_1.OffChainAddressVersion.testnet, transactions_2.getPublicKey(subdomainOptions.newOwnerKeyPair.privateKey));
    ...
    return did_1.encodeStacksV2Did({
        anchorTxId: txId,
        address: transactions_2.publicKeyToAddress(constants_1.OffChainAddressVersion.testnet, transactions_2.getPublicKey(subdomainOptions.newOwnerKeyPair.privateKey)),
    });
};
exports.rekeySubdomain = rekeySubdomain;
const revokeSubdomain = async (fqn, nameOwnerKey, network) => {
    const revokeAddr = constants_1.SUBDOMAIN_REVOKED_ADDR;
