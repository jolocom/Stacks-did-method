...
const findValidNames = (onlyMigrated = false, ignoreExpired = false) => (page = 0) => {
    return api_1.fetchAllNames(page).pipe(fluture_1.map(ramda_1.map((fqn) => {
        const { name, namespace } = general_1.decodeFQN(fqn);
        return api_1.fetchNameInfo({ name, namespace }).pipe(fluture_1.chain((info) => {
            ...
            return api_1.fetchZoneFileForName({
                name,
                namespace,
