...
//@ts-ignore
bns_1.mapDidToBNSName(did, network)
    .pipe(fluture_1.chain(({ name, namespace, subdomain, tokenUrl }) => signedToken_1.fetchAndVerifySignedToken(tokenUrl, did.address).pipe(fluture_1.map((key) => ({
    publicKey: key,
    name: general_1.encodeFQN({ name, namespace, subdomain }),
})))));
const postResolve = (name, did, network) => {
    const fqn = general_1.decodeFQN(name);
    return api_1.fetchNameInfo(network.coreApiUrl)(fqn).pipe(fluture_1.chain((currentInfo) => {
        ...
        return api_1.getCurrentBlockNumber(network.coreApiUrl)
            ...
            .pipe(fluture_1.mapRej((err) => new Error(`PostResolution: failed to fetch latest public key, error: ${err.message}`))).pipe(fluture_1.map((publicKey) => ({ publicKey, did })));
    }));
};
const getResolver = (stacksNetwork = new network_1.StacksMainnet()) => {
    const resolve = (did) => fluture_1.promise(did_1.parseStacksV2DID(did)
        .map((parsedDID) => (did_1.isMigratedOnChainDid(parsedDID) // TODO Check if it matches network, i.e. mainnet vs testnet
