...
import { encaseP, map, resolve, chain, reject, FutureInstance, fork } from "fluture"
import { encodeFQN, stripHexPrefixIfPresent } from "./utils/general"
import { BNS_ADDRESSES } from "./constants"
import { addressToString, bufferCVFromString, callReadOnlyFunction, ClarityAbiTypeTuple, cvToJSON, hexToCV, parseToCV, ReadOnlyFunctionOptions, serializeCV, TupleCV, tupleCV } from "@stacks/transactions"
...
export const fetchNameInfo = ({
  ...
  return fetchJSON<NameInfo>(endpoint)
    .pipe(chain(res => {
        return fetchNameInfoFromContract({name, namespace, network: new StacksMocknet()})
          .pipe(map(({address, zonefile_hash}) => {
            if (stripHexPrefixIfPresent(zonefile_hash) === stripHexPrefixIfPresent(res.zonefile_hash)) {
              return {
                ...res,
                address
              }
...
export const fetchNameInfoFromContract = ({
  ...
  //@ts-ignore
  return encaseP(callReadOnlyFunction)(options).pipe(chain(v => {
...
export const fetchTransactionById = (txId: string) => {
  ...
  return fetchJSON<{ tx_id: string; tx_status: TxStatus }>(endpoint).pipe(chain(res => {
